...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s SAFE too low! (Should be %s SAFE)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s SAFE too high! (Should be %s SAFE)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 SAFE from 0 to 2 using sendtoaddress call.
        ...
        self.nodes[2].freezeunspent(False, [unspent_0])
        assert_raises(JSONRPCException, self.nodes[2].sendtoaddress, self.nodes[2].getnewaddress(), 200)
        assert_equal([unspent_0], self.nodes[2].listfrozenunspent())
        self.nodes[2].freezeunspent(True, [unspent_0])
        assert_equal(len(self.nodes[2].listfrozenunspent()), 0)

        # Have node1 generate 100 blocks (so node0 can recover the fee)
        self.nodes[1].generate(100)
        self.sync_all()

        # node0 should end up with 1000 SAFE in block rewards plus fees, but
        ...
        # Send 100 SAFE with subtract fee from amount
        ...
        # Sendmany 100 SAFE with subtract fee from amount
