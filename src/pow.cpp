...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN || Params().NetworkIDString() == CBaseChainParams::REGTEST) {
        if (pindexLast->nHeight + 1 >= 34140)
        {
            if(pindexLast->nHeight + 1 >= g_nCriticalHeight && pindexLast->nHeight + 1 < g_nCriticalHeight + g_nAnWwangDiffOffset) // anwang-BTC
                retarget = DIFF_BTC;
            else if(pindexLast->nHeight + 1 >= g_nCriticalHeight + g_nAnWwangDiffOffset && pindexLast->nHeight + 1 < g_nCriticalHeight + 2 * g_nAnWwangDiffOffset) // anwang-KGW
                retarget = DIFF_KGW;
            else
                retarget = DIFF_DGW;
        }
