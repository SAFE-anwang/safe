	...
	const std::vector<CDeterministicMasternode_IndexValue> &vtResultDeterministicMN)
{
	...
	int64_t nTempTime = GetTimeMillis();
	...
	while (nActualTimeMillisInterval > 500)
	{
		boost::this_thread::interruption_point();
		MilliSleep(50);
		nTempTime = GetTimeMillis();
		nActualTimeMillisInterval = std::abs(nNextBlockTime - nTempTime);
	}
...
void static SposMiner(const CChainParams& chainparams, CConnman& connman)
{
	...
	while (true)
	{
		...
		{			
			...
			{
				if (!bTimeLog && nCurTime + 5 < (int64_t)pTopBlock->nTime)
				{
					bTimeLog = true;
