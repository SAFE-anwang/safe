...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Safe address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no safe: URI
    if(!uri.isValid() || uri.scheme() != QString("safe"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because safe:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("safe://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "safe:");
    }
...
{
    QString ret = QString("safe:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open safe.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Safe Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Safe Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Safe Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Safe Core*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "safe.desktop";
    return GetAutostartDir() / strprintf("safe-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a safe.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Safe Core\n";
        else
            optionFile << strprintf("Name=Safe Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Safe Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Safe Core app to startup item list
...
void restoreWindowGeometry(const QString& strSetting, const QSize& defaultSize, QWidget *parent)
{
    ...
    if ((!pos.x() && !pos.y()) || (QApplication::desktop()->screenNumber(parent) == -1))
    {
        ...
        //defaultPos cannot be calculated with defaultsize, because the final size may be larger than defaultsize
        QPoint defaultPos = screen.center() -
            QPoint(MAIN_WINDOW_NORMAL_WIDTH / 2, MAIN_WINDOW_NORMAL_HEIGHT / 2);
