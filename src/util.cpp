...
const char * const BITCOIN_CONF_FILENAME = "safe.conf";
const char * const BITCOIN_PID_FILENAME = "safed.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "safe" is a composite category enabling all Safe-related debug output
            if(ptrCategory->count(string("safe"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Safe_Spos2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Safe_Spos2
    // Mac: ~/Library/Application Support/Safe_Spos2
    // Unix: ~/.safe_spos2
#ifdef WIN32
    // Windows
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty safe.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override safe.conf
